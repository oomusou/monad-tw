        ...
        <div class="h-full relative" v-for="edge in records" :key="edge.node.id">
          <g-link :to="edge.node.path">
            <div class="h-full relative" :id="edge.node.id">
              <g-image :src="edge.node.image" :alt="edge.node.title" class="rounded-lg object-cover absolute -z-10 h-full w-full"></g-image>

              <div class="featured-label absolute top-0 right-0 pr-10 pt-10 z-10">
                <span class="p-1 relative block text-center text-xs leading-tight bg-white border border-gray-300 text-black rounded-full">
